...
/**全局变量*/
Mat srcImage, dstImage, grayImage, maskImage;//定义原始图、目标图、灰度图、掩模图
int g_nFillMode = 1;//1则是fixed_range
...
int main( int argc, char** argv )
{
    ...
    //复制到dstImage
    srcImage.copyTo(dstImage);//拷贝源图到目标图
    //转换成grayImage
    cvtColor(srcImage, grayImage, COLOR_BGR2GRAY);//转换三通道的image0到灰度图
    //生成空的maskImage
    maskImage.create(srcImage.rows+2, srcImage.cols+2, CV_8UC1);//利用image0的尺寸来初始化掩膜mask
        ...
        switch( (char)c )
        {
            ...
            case '1':
                if( g_bIsColor )//若原来为彩色，转为灰度图，并且将掩膜mask所有元素设置为0
                {
                    ...
                    maskImage = Scalar::all(0);	//将mask所有元素设置为0
